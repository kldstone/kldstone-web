...
export default function Process() {
  useEffect(() => {
    const howToSteps = steps.map((step, i) => ({
      ...
      position: i + 1,
      name: step.title,
      text: step.desc.replace(/\n/g, " "),
    }));

    const jsonLd = {
      "@context": "https://schema.org",
      "@type": "HowTo",
      name: "天然石材加工制造流程：从荒料到出口交付",
      description: "康利德石材工厂的完整加工制造流程，涵盖荒料选材、大板切割、水刀拼花、CNC异形加工、表面处理、预拼验收、品质质检、包装加固、装柜发运到出口交付共十道工序。",
      totalTime: "P15D",
      step: howToSteps,
    };
    ...
    script.id = "schema-howto";
    script.textContent = JSON.stringify(jsonLd);
    document.head.appendChild(script);
    return () => { const s = document.getElementById("schema-howto"); s?.remove(); };
  }, []);
